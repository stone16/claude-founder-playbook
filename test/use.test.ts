...
describe("founder use", () => {
  ...
  it("ignores workspace flags before the idea slug", async () => {
    const workspace = await mkdtemp(path.join(tmpdir(), "founder-use-prefix-workspace-"));
    vi.spyOn(console, "log").mockImplementation(() => undefined);
    await main(["new", "Contract Review Tool", "--workspace", workspace]);
    await main(["new", "Another Tool", "--workspace", workspace]);

    expect(await main(["use", "--workspace", workspace, "contract-review-tool"])).toBe(0);
